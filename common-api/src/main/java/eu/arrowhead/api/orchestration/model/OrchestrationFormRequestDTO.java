...
public class OrchestrationFormRequestDTO implements Serializable {
	...
	public OrchestrationFormRequestDTO validateCrossParameterConstraints() {
        if (requestedService == null && orchestrationFlags.get(OrchestrationFlags.Flag.OVERRIDE_STORE)) {
            throw new BadPayloadException("Requested service can not be null when \"" + OrchestrationFlags.Flag.OVERRIDE_STORE + "\" is TRUE");
        }
        
        if (requestedService == null && orchestrationFlags.get(OrchestrationFlags.Flag.TRIGGER_INTER_CLOUD)) {
            throw new BadPayloadException("Requested service can not be null when \"" + OrchestrationFlags.Flag.TRIGGER_INTER_CLOUD + "\" is TRUE");
        }

        if (orchestrationFlags.get(OrchestrationFlags.Flag.ONLY_PREFERRED)) {
        ...
        if (orchestrationFlags.get(OrchestrationFlags.Flag.ENABLE_QOS) && commands.containsKey(QOS_COMMAND_EXCLUSIVITY) && !orchestrationFlags.get(
                OrchestrationFlags.Flag.MATCHMAKING)) {
	...
	public static class Builder {
		...
		//-------------------------------------------------------------------------------------------------
		public Builder flag(final OrchestrationFlags.Flag flag, final boolean value) {
