...
public class HttpService {
    ...
    public HttpService(final SSLProperties sslProperties) {
        this.sslProperties = sslProperties;
        this.errorHandler = new ArrowheadHttpClientResponseErrorHandler();
        this.disableHostnameVerifier = Defaults.DEFAULT_DISABLE_HOSTNAME_VERIFIER;
        this.connectionTimeout = Defaults.DEFAULT_CONNECTION_TIMEOUT;
        this.socketTimeout = Defaults.DEFAULT_SOCKET_TIMEOUT;
        this.connectionManagerTimeout = Defaults.DEFAULT_CONNECTION_MANAGER_TIMEOUT;
    }
    ...
    public <T, P> ResponseEntity<T> sendRequest(final UriComponents uri, final HttpMethod method, final Class<T> responseType, final P payload,
        ...
        try {
            ...
            if (ex.getMessage().contains(ERROR_MESSAGE_PART_PKIX_PATH)) {
                ...
                throw new AuthException("The system at " + uri.toUriString() + " is not part of the same certificate chain of trust!",
                ...
                logger.error("The certificate of the system at {} does not contain the specified IP address or DNS name as a Subject Alternative Name.",
                             uri.toString());
                throw new AuthException("The certificate of the system at " + uri
